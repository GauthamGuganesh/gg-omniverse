package design.pattern.behavioural.state;
...
// plural because Spring State Machine defines a class called State already
enum States
{
  OFF_HOOK, // starting
  ON_HOOK, // terminal
...
class SpringStatemachineDemo
{
    ...
    throws Exception
  {
    ...
    builder.configureStates()
      .withStates()
        ...
        .states(EnumSet.allOf(States.class)); //initial states.

    builder.configureTransitions()
      .withExternal()
        .source(States.OFF_HOOK)
        .event(Events.CALL_DIALED)
        .target(States.CONNECTING) //Source events and the triggers that land them in target events.
        .and()
      .withExternal()
        .source(States.OFF_HOOK)
  ...
  public static void main(String[] args) throws Exception
  {
    ...
    while (true)
    {
      State<States, Events> state = machine.getState(); //Getting current state.

      System.out.println("The phone is currently " + state.getId());
      System.out.println("Select a trigger:");

      List<Transition<States, Events>> ts = machine.getTransitions() //getTransitions gives all triggers
        .stream()
        .filter(t -> t.getSource() == state) //Getting triggers for the current state alone so that user can select one.
      ...
      Events event = ts.get(choice).getTrigger().getEvent();
      machine.sendEvent(event); //Sending trigger to change state
