...
//Dummy database - Injected instead of live one.
...
class SingletonRecordFinder //Dependent
{ 
  private Database db;
  ...
  public int getTotalPopulation(List<String> names)
  {
    ...
    for (String name : names)
      result += db.getPopulation(name); // instead of tight-coupling with SingletonDatabase.getInstance().getPopulation(name);
    return result;
  }
}

class ConfigurableRecordFinder //Dependent
{
  private Database database;

  public ConfigurableRecordFinder(Database database) {
    this.database = database; // Dependency injection. We can inject live database or dummy database.
  }
...
class Testability
{
  ...
  {
	  //Injecting our own database instead of live.
