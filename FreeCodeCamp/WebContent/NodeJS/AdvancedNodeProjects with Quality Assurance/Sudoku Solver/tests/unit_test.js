...
suite('UnitTests', () => {
  ...
  suite('Function ____()', () => {
    test('Valid "1-9" characters', (done) => {
      ...
      let valid = input.every(ele => Solver.isContentAllowed(ele));
  ...
  suite('Function ____()', () => {
    test('Parses a valid puzzle string into an object', done => {
      ...
      let solution = Solver.solveSudoku(input);
    ...
    test('Shows an error for puzzles that are not 81 numbers long', done => {
      ...
      let err  = Solver.renderTextToGrid(shortStr);
      let err2 = Solver.renderTextToGrid(longStr);
  ...
  suite('Function ____()', () => {
    ...
    test('Valid puzzles pass', done => {
      ...
      assert.equal(Solver.isPuzzlePass(input), true);
      done();
    });
    ...
    test('Invalid puzzles fail', done => {
      ...
      assert.equal(Solver.isPuzzlePass(input), false);
      done();
    });
  });
  ...
  suite('Function ____()', () => {
    ...
    test('Returns the expected solution for an incomplete puzzle', done => {
      const input = '..9..5.1.85.4....2432......1...69.83.9.....6.62.71...9......1945....4.37.4.3..6..';
      assert.equal(Solver.solveSudoku(input), '769235418851496372432178956174569283395842761628713549283657194516924837947381625');
      done();
    });
  });
});
